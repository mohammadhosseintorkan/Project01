...
$result = $db->execute($sql);
if (isset($result))
    Alert::alerts('جدول مقالات با موفقیت ایجاد شد', 'success');
...
$sql = "CREATE TABLE IF NOT EXISTS LikeBlog( 
		id INT AUTO_INCREMENT NOT NULL,
		Userid INT,
		Blogid INT,		
		status VARCHAR(20),
...
$result = $db->execute($sql);
if (isset($result))
    Alert::alerts('جدول لایک مقالات با موفقیت ایجاد شد', 'success');
